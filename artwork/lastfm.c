...
#define API_KEY "22b98cbd127b991d34458fc42863d75d"
#define MEGA_IMAGE_TAG "<image size=\"mega\">"
#define XL_IMAGE_TAG "<image size=\"extralarge\">"
#define IMAGE_END_TAG "</image>"
int fetch_from_lastfm (const char *artist, const char *album, char *dest, const int dest_len) {
...
//    trace("fetch_from_lastfm: scrobbler response:\n%s\n", buffer);
    ...
    *end = '\0';
    ...
    if (end - img > dest_len) {
        trace("fetch_from_lastfm: dest is too short\n");
        return -1;
    }
    strcpy(dest, img);
    return end - img;
